...
**الإصدار:** Worker `v1.3.1` · الواجهة `v1.3.0`   ← الاتنين مستقلين، طبيعي يختلفوا
...
> القيم دي مسجّلة **جنب الكود** في `log-values.json` (مش في `ecommoda-constants` —
> الجدول هناك اتنقل §7.1). `node check-log-values.mjs` بيتحقق منها آليًا.

### الحارس الديناميكي لقيم اللوج (الطبقة ٥)

الأداة فيها الحارس بتاع `ecommoda-worker-builder` Step 7-ج جوّه `writeLog`
(الدالة الوحيدة اللي بتكتب لوج هنا — مفيش `writeLogsBatch`). أي زوج
`(tool, type)` مش مسجّل في `LOG_REGISTRY` (المبني من `log-values.json`)
بيتكتب **عادي** + `extra._unregistered = true` + UPSERT صامت في
`log_value_alerts` المشترك — **مفيش رفض كتابة أبدًا**.
